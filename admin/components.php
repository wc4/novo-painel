...
if (count($componentsec) != 0) {
	foreach ($componentsec as $component) {
		...
		$table .= '<a href="#" title="'.i18n_r('DELETE_COMPONENT').': '. cl($component->title).'?" class="delcomponent" rel="'.$count.'" ><span class="lnr lnr-trash"></span></a></td></tr></table>';
		$table .= '<textarea name="val[]" class="form-control" rows="10">'. stripslashes($component->value) .'</textarea>';
	...
	if($count > 1) {
		$item = 0;
		foreach($componentsec as $component) {
			$listc .= '  <a id="divlist-' . $item . '" href="#section-' . $item . '" class="component label label-primary">' . $component->title . '</a>';
			$item++;
		}
	} elseif ($count == 0) {
		$submitclass = 'hidden';
						...
						<div class="panel-body">
							<div class="row">
								<div class="col-md-9" id="maincontent">									
									<div class="edit-nav" >
										<a href="#" id="addcomponent" accesskey="<?php echo find_accesskey(i18n_r('ADD_COMPONENT'));?>" class="btn btn-primary btn-sm"><?php i18n('ADD_COMPONENT');?></a>
										<div class="clear"></div>
									</div>
									...
									<form class="manyinputs" action="<?php myself(); ?>" method="post" accept-charset="utf-8" >
										...
										<br>
											<span><input type="submit" class="submit btn btn-primary" name="submitted" id="button" value="<?php i18n('SAVE_COMPONENTS');?>" /></span> &nbsp;&nbsp;<?php i18n('OR'); ?>&nbsp;&nbsp; <a class="cancel" href="components.php?cancel"><?php i18n('CANCEL'); ?></a>
										</p>
									</form>
								</div>
								<div class="col-md-3">
